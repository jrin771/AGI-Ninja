...
def find_and_draw_pose(pose, frame, background) -> tuple:
    ...
    # Filter out only hand landmarks
    if results.pose_landmarks:
        hand_landmarks_indices = [17, 18, 19, 20, 21]  # Indices corresponding to hands in PoseLandmark

        # Create a new PoseLandmarks object
        filtered_landmarks = landmark_pb2.NormalizedLandmarkList()
        for idx in hand_landmarks_indices:
            filtered_landmarks.landmark.extend([results.pose_landmarks.landmark[idx]])
